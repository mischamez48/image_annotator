...
class ImageAnnotator(QMainWindow):
    def __init__(self):
        ...
        self.image_label.set_prompt_modality("Bounding Box")  # Initialize with default prompt modality
    ...
    def apply_sam_prediction(self):
        prompt_type = self.image_label.prompt_modality
        ...
        if prompt_type == "Bounding Box":
            ...
            prediction = self.sam_utils.apply_sam_prediction(self.current_image, prompt_type, bbox=bbox)
        
        elif prompt_type == "Points":
            if not self.image_label.sam_points:
                print("No SAM points provided")
                return
            print(f"Applying SAM prediction with points: {self.image_label.sam_points}")
            prediction = self.sam_utils.apply_sam_prediction(self.current_image, prompt_type, points=self.image_label.sam_points)
        
        elif prompt_type == "Mask":
            if self.image_label.sam_mask is None:
                print("No SAM mask provided")
                return
            print(f"Applying SAM prediction with mask")
            prediction = self.sam_utils.apply_sam_prediction(self.current_image, prompt_type, mask=self.image_label.sam_mask)
    ...
    def accept_sam_prediction(self):
        if self.image_label.temp_sam_prediction:
            ...
            # Clear prompts after accepting prediction
            if self.image_label.prompt_modality == "Points":
                self.image_label.sam_points = []
            elif self.image_label.prompt_modality == "Mask":
                self.image_label.sam_mask = None
            elif self.image_label.prompt_modality == "Bounding Box":
                self.image_label.sam_bbox = None
            
            self.image_label.update()
            print("SAM prediction accepted and added to annotations.")
    
    def annotation_to_polygon(self, annotation):
        """Convert any annotation type to a Shapely Polygon."""
        ...
        if 'segmentation' in annotation:
            ...
            rectangle_points = [(x, y), (x + w, y), (x + w, y + h), (x, y + h)]
    ...
    def setup_tool_buttons(self):
        ...
        self.sam_model_selector.currentTextChanged.connect(self.on_sam_model_changed)
        automated_layout.addWidget(self.sam_model_selector)
    
        # Prompt modality selector
        prompt_label = QLabel("Prompt Type:")
        prompt_label.setAlignment(Qt.AlignCenter)
        automated_layout.addWidget(prompt_label)
        
        self.prompt_modality_selector = QComboBox()
        self.prompt_modality_selector.addItems(["Bounding Box", "Points", "Mask"])
        self.prompt_modality_selector.setCurrentText("Bounding Box")
        self.prompt_modality_selector.currentTextChanged.connect(self.on_prompt_modality_changed)
        automated_layout.addWidget(self.prompt_modality_selector)
    
        automated_buttons_layout = QHBoxLayout()
        self.sam_magic_wand_button = QPushButton("SAM-Assisted")
        self.sam_magic_wand_button.setCheckable(True)
        self.sam_magic_wand_button.setEnabled(False)  # Initially disabled
    ...
    def setup_sidebar(self):
        ...
        # Add prompt modality selector
        prompt_label = QLabel("Prompt Type:")
        prompt_label.setAlignment(Qt.AlignCenter)
        sam_layout.addWidget(prompt_label)
        
        self.prompt_modality_selector = QComboBox()
        self.prompt_modality_selector.addItems(["Bounding Box", "Points", "Mask"])
        self.prompt_modality_selector.setCurrentText("Bounding Box")
        self.prompt_modality_selector.currentTextChanged.connect(self.on_prompt_modality_changed)
        sam_layout.addWidget(self.prompt_modality_selector)
    ...
    def on_prompt_modality_changed(self, modality):
        """Handle prompt modality selection change."""
        self.image_label.set_prompt_modality(modality)
        print(f"Changed prompt modality to: {modality}")
...
###################################################################
    ...
    def merge_annotations(self):
        ...
        for item in selected_items:
            ...
            # Convert annotation to polygon using utility function
            polygon = self.annotation_to_polygon(annotation)
            if polygon is not None:
                polygons.append(polygon)
            else:
                # Skip annotations without valid geometry
                print(f"Warning: Annotation {annotation.get('number', 'unknown')} has no valid geometry data")
                continue
        
        # Check if we have enough valid polygons after conversion
        if len(polygons) < 2:
            QMessageBox.warning(self, "Not Enough Valid Annotations", 
                              "Not enough annotations with valid geometry data to merge.")
            return
        ...
        try:
            ...
            print(f"Merge successful. Result type: {type(merged_polygon)}")
        except Exception as e:
