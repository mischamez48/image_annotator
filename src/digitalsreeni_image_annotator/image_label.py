...
class ImageLabel(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        # Prompt modalities
        self.prompt_modality = "Bounding Box"  # "Bounding Box", "Points", "Mask"
        self.sam_points = []  # List of (x, y, label) tuples where label is 1 (positive) or 0 (negative)
        self.sam_mask = None  # Input mask for dense prompting
        self.drawing_sam_points = False
        self.drawing_sam_mask = False
        self.temp_sam_points = []
    ...
    def set_prompt_modality(self, modality):
        """Set the current prompt modality for SAM."""
        self.prompt_modality = modality
        # Only clear prompts that are not compatible with the new modality
        if modality == "Bounding Box":
            self.sam_points = []
            self.sam_mask = None
            self.drawing_sam_points = False
            self.drawing_sam_mask = False
    ...
    def clear_current_prompts(self):
        """Clear prompts for the current modality only."""
        if self.prompt_modality == "Points":
            self.sam_points = []
        elif self.prompt_modality == "Mask":
            self.sam_mask = None
            self.temp_sam_points = []
        elif self.prompt_modality == "Bounding Box":
            self.sam_bbox = None
        self.update()
    
    def create_mask_from_points(self):
        """Create a mask from the drawn points."""
        if not self.temp_sam_points:
            return
        ...
        points = np.array([(int(p[0]), int(p[1])) for p in self.temp_sam_points], dtype=np.int32)
        ...
        if len(points) >= 3:
            ...
            # Draw a small circle if only 1 point
            cv2.circle(mask, tuple(points[0]), 5, 255, -1)
    ...
    def paintEvent(self, event):
        ...
        if self.scaled_pixmap:
            ...
            if self.sam_magic_wand_active:
                ...
                if self.sam_points:
                    self.draw_sam_points(painter)
                if self.drawing_sam_mask and self.temp_sam_points:
                    self.draw_temp_sam_mask(painter)
                
                # Draw status message for Points modality
                if self.prompt_modality == "Points" and self.sam_points:
                    self.draw_points_status_message(painter)
    ...
    def draw_sam_points(self, painter):
        """Draw SAM points (positive and negative)."""
        painter.save()
        painter.translate(self.offset_x, self.offset_y)
        painter.scale(self.zoom_factor, self.zoom_factor)
        ...
        for x, y, label in self.sam_points:
            if label == 1:  # Positive point
                painter.setPen(QPen(Qt.green, 3 / self.zoom_factor, Qt.SolidLine))
                painter.setBrush(QBrush(Qt.green))
            else:  # Negative point
                painter.setPen(QPen(Qt.red, 3 / self.zoom_factor, Qt.SolidLine))
    ...
    def draw_temp_sam_mask(self, painter):
        """Draw temporary SAM mask while drawing."""
        painter.save()
        painter.translate(self.offset_x, self.offset_y)
        painter.scale(self.zoom_factor, self.zoom_factor)
        
        if len(self.temp_sam_points) > 1:
            # Draw lines connecting the points
            painter.setPen(QPen(Qt.blue, 2 / self.zoom_factor, Qt.SolidLine))
            for i in range(len(self.temp_sam_points) - 1):
                p1 = self.temp_sam_points[i]
                p2 = self.temp_sam_points[i + 1]
                painter.drawLine(QPointF(p1[0], p1[1]), QPointF(p2[0], p2[1]))
        
        # Draw points
        painter.setPen(QPen(Qt.blue, 3 / self.zoom_factor, Qt.SolidLine))
        painter.setBrush(QBrush(Qt.blue))
        for point in self.temp_sam_points:
            radius = 3 / self.zoom_factor
            painter.drawEllipse(QPointF(point[0], point[1]), radius, radius)
        
        painter.restore()
    
    def draw_points_status_message(self, painter):
        """Draw status message for Points modality."""
        painter.save()
        ...
        message = f"Points: {len(self.sam_points)} | Press Enter to accept, Esc to clear, C to clear points"
        metrics = painter.fontMetrics()
        text_rect = metrics.boundingRect(message)
        
        # Position at top-left of image
        x = int(self.offset_x + 10)
        y = int(self.offset_y + 30)
        
        # Draw background
        painter.setPen(QPen(Qt.black, 1))
        painter.setBrush(QBrush(QColor(0, 0, 0, 128)))  # Semi-transparent black
        painter.drawRect(x - 5, y - text_rect.height() - 5, text_rect.width() + 10, text_rect.height() + 10)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.modifiers() == Qt.ControlModifier and event.button() == Qt.LeftButton:
            ...
            if event.button() == Qt.LeftButton:
                if self.sam_magic_wand_active:
                    if self.prompt_modality == "Bounding Box":
                        self.sam_bbox = [pos[0], pos[1], pos[0], pos[1]]
                        self.drawing_sam_bbox = True
                    elif self.prompt_modality == "Points":
                        # Add positive point
                        self.sam_points.append((pos[0], pos[1], 1))
                        # Trigger SAM prediction after adding points
                        if self.main_window:
                            self.main_window.apply_sam_prediction()
                    elif self.prompt_modality == "Mask":
                        # Start drawing mask
                        self.drawing_sam_mask = True
                        self.temp_sam_points = [pos]
            ...
            elif event.button() == Qt.RightButton and self.sam_magic_wand_active:
                if self.prompt_modality == "Points":
                    # Add negative point
                    self.sam_points.append((pos[0], pos[1], 0))
                    # Trigger SAM prediction after adding points
                    if self.main_window:
                        self.main_window.apply_sam_prediction()
        self.update()
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if event.modifiers() == Qt.ControlModifier and event.buttons() == Qt.LeftButton:
            ...
            pos = self.cursor_pos
            if self.sam_magic_wand_active:
                if self.drawing_sam_bbox:
                    self.sam_bbox[2] = pos[0]
                    self.sam_bbox[3] = pos[1]
                elif self.drawing_sam_mask and event.buttons() == Qt.LeftButton:
                    self.temp_sam_points.append(pos)
            elif self.editing_polygon:
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        elif event.key() == Qt.Key_C and self.sam_magic_wand_active:
            # Clear current prompts with C key
            self.clear_current_prompts()
            print("Cleared current prompts")
        elif event.key() == Qt.Key_Escape:
            if self.temp_annotations:
                self.discard_temp_annotations()
            elif self.sam_magic_wand_active:
                # Clear prompts based on current modality
                if self.prompt_modality == "Points":
                    self.sam_points = []
                elif self.prompt_modality == "Mask":
                    self.sam_mask = None
